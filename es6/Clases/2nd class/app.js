...
var innerKeys = Object.keys(mobiles);
...
function searchModel() {
  ...
  for (var i = 0; i < innerInnerKeys.length; i++) {
...
function getData() {
  ...
  div.innerHTML += `<div>
    <h2>${modelValue.brand}</h2>
    <h3>${modelValue.operatingSystem}</h3>
    <h4>${modelValue.processor}</h4>
...
let result = isActive || "executed...";

console.log(result);

let exp = (1 && 10 + 20) || undefined;

let exp = "" || (null + (10 - 5) && "faraz") || 1 || " ";

let exp = 100 && (0 + 1) || true || undefined && 10;
...
let [num1, num2, num3] = city3;

console.log(num2);

destructing of object

let {id,name: std_name,city} = {
  id: 2,
  name: "abc",
  city: "karachi",
};
...
function getStudentData({ id, name, age, email }) {
  console.log(id, name, age, email);
}

getStudentData({ id: 222, name: "faraz", age: 25, email: "faraz@gmail.com" });
...
function sum(a, b, ...rest) {
  ...
  for (let i = 0; i < rest.length; i++) {
    remainNumbers += rest[i];
  }

  console.log(a + b + remainNumbers);
